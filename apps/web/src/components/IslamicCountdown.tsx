...
import { getUmmAlQuraHijri, getUmmAlQuraMonthStart } from "@hilal/astronomy";
...
export function IslamicCountdown() {
    ...
    // use primitive for dependency array comparison
    const todayNum = today.getTime();

    const events = useMemo(() => computeCountdowns(new Date(todayNum)), [todayNum]);
            ...
            {events.map((event) => {
                ...
                return (
                    ...
                    >
                        ...
                        <div
                            className="text-[10px] font-bold uppercase tracking-wider mb-0.5 text-muted-foreground"
                        >
                            {event.name}
                        </div>
                        <div
                            className="text-[9px] mb-2 text-muted-foreground font-serif"
                        >
                            {event.nameAr}
                        </div>
                        <div
