...
A **persistent date/location chip** (glassmorphic pill) floats below the header on all pages except Home, showing the currently selected city name and date. This ensures users always know their active context while navigating.
...
Both views share: **Criterion toggle** (Yallop/Odeh), **GPS auto-detect**, **city search** (Open-Meteo geocoding), **cloud cover overlay** (Open-Meteo, independently toggleable), **Atmospheric Overrides** panel (Pro — temperature °C, pressure hPa, elevation m), **Best Time to Observe** card (Pro — optimal sunset window), **DEM Integration** (Open-Meteo elevation API on map click), **Enhanced Tooltips** (q-value, moon age, altitude, elongation, crescent width, terrain elevation), and **High Contrast CVD mode**. Touch interactions use `touchAction: "none"` for smooth dragging on mobile devices.

The **visibility legend panel** (`MapLegendPanels.tsx`) displays criterion-specific zone labels — `YALLOP_ZONE_LABELS` shows q-value thresholds (e.g., q ≥ +0.216 for Zone A) while `ODEH_ZONE_LABELS` shows V-value thresholds (e.g., V ≥ 5.65 for Zone A). The legend updates dynamically when the user toggles between criteria.
...
A **Crescent Country List** panel (`CrescentCountryList.tsx`) in the sidebar computes visibility for each city in `MAJOR_CITIES` (~90 cities) on the 29th night of the displayed Hijri month, classifies them using the selected criterion, and groups countries by visibility zone (A–F) as collapsible `<details>` sections with country chips. This provides at-a-glance regional crescent predictions — e.g., "Zone A countries that can easily see the new crescent by naked eye on the 29th night."
...
| 42 | UX Snag Fixes & Resilience | **Language dropdown fix:** Removed `overflow-hidden` from header that clipped the language dropdown. **Moon icon:** Gold colour + glow on Home hero button. **Map/Globe drag:** Added `touchAction: "none"` to map and globe containers for mobile drag support. **Horizon mobile scaling:** Responsive scale factor for all canvas fonts, glow radii, and label spacing (scales down to 60% on narrow screens). **Legend per criterion:** Added `YALLOP_ZONE_LABELS` and `ODEH_ZONE_LABELS` to `shared/astronomy.ts` with threshold strings; `MapLegendPanels` now displays criterion-specific labels dynamically. **Persistent date/location chip:** Glassmorphic floating pill below header on all pages (except Home) showing current city + date. **Crescent Country List:** New `CrescentCountryList.tsx` component on Calendar page — computes visibility for `MAJOR_CITIES` on the 29th night and groups countries by zone. **tRPC resilience:** Custom fetch wrapper in `main.tsx` detects non-JSON error responses from Vercel cold-start crashes and wraps them in valid tRPC error JSON. Added retry (2×) for transient JSON parse failures. Filtered transient server errors from Sentry. |
| 43 | Serverless Resilience & UX Scrolling Fixes | Fixed Vercel `FUNCTION_INVOCATION_FAILED` timeouts originating from database connection pool buildup and Open-Meteo external fetches. Lowered `connectionLimit` and enforced strict `connectTimeout: 5000` inside `db.ts`. Disabled React Query `refetchOnWindowFocus` for heavy db telemetry calls in `SightingFeed.tsx`. Fixed global `lg:overflow-hidden` traps that prohibited the Visibility pages from natively scrolling on desktop browsers. Corrected `visibility.worker.ts` web-worker overlay jumping by anchoring the phase offset grid calculation specifically against localized UTC. Drafted `.agents/workflows/hilal-vision-troubleshooter.md` diagnostic tree for quick resolution of API/Dev-Server hangs. |
| 44 | Timezone Projection & Vercel Timeout Hardening | Completed Phase 2 of Serverless harding: explicitly disabled `refetchOnWindowFocus` in `MapPage`, `BestTimeCard`, and `HorizonPage` tRPC hooks to prevent concurrent exhaustion of Vercel memory pooling when users tab-switch. Handled the 2D/3D map jumping artifact permanently by forcing `visibility.worker.ts` to calculate crescent widths securely against Local Noon UTC offsets rather than Client Midnight, eliminating the dateline day-shifting paradoxes. |
...
*Documentation updated March 3, 2026 (Round 43 — Resolving severe Serverless db pool hangs on Vercel endpoints and resolving viewport scrolling regressions on Map/Visibility layouts).*
